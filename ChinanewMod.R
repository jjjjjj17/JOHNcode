...
data <- read.csv("C:/Chinanew/Modified_China.csv", header=TRUE)

# 要标准化的列的索引，假设是从第4列到第9列
columns_to_normalize <- 3:92
...
# 将结果写入新的CSV文件
write.csv(data, file = "C://Chinanew//proteinnormalizeall.csv", row.names = FALSE)